...
function list() {
    ...
    //Modal
    var btn_frame = `<button type="button" class="btn btn-info btn-xs" data-toggle="modal" data-target="#myModal">Open Large Modal</button>`;
    btn_panel.insertAdjacentHTML('beforeend', btn_frame);
...
function ctable(tb) {
    ...
    var btn_panel = document.getElementsByClassName("navbar-header")[0];
    var btn_frame = `<div class="navbar-header" style="text-align:right;background-color:cornflowerblue"><div class="" style="right:0"><a class="btn btn-default  btn-xs navbar-brand " onclick="tg2()">預覽課表</a></div></div>`;
    btn_panel.insertAdjacentHTML('afterend', btn_frame);
    $(".w3-sidebar").hide();
}
